...
{//评估某一点的估价
    ...
    number=0;
    empty1=0;
    for(i=x-1,j=y-1;i>=0&&j>=0&&i>x-6&&j>y-6;--i,--j)//从(x,y)位置往左写对角线向左上遍历计数连续的turn的个数
    ...
    scoree+=scoretable(number+1,empty1);//计算以包含(x,y)连续的turn棋子的个数以求得是哪种类型，求得估价函数
    number=0;
    empty1=0;
